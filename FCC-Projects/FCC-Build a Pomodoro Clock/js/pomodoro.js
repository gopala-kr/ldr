$(document).ready(function () {
  var $up = $('.plus'),  // plus button
    ...
    $minutes = $('.clock-wrapper .clock .minutes'),  // minutes thenselves
  ...
  function startTimer() {
    ...
    if(breakbool) { // if were on break need to offset the minus that will happen from restart
      minutes++
      breakbool = false;
    };
    minutes--;
    $minutes.text(minutes); // this changes the minutes right away
    changeTimer(); // called first so there is no visable delay in the start and when the timer starts
